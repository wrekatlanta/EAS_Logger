...
class	Month:
	...
	def 	CheckIf_RmtOutsideWeekRange(self, eas_handles, week, requested_week, trace_flag):
		if self.RmtCheckNecessary(requested_week) == True:
			if trace_flag == True:
			...
			if alert.curr_date.month == requested_week.sunday.month:
				if trace_flag == True:
					eas_handles.WriteTo_TraceFile("\nRMT check month found!\n")
...
def	Perform_Trace_Extraction( week, alert, eas_handles, month,requested_week):
	...
	if month.RmtCheckNecessary(requested_week) == True:
...
def	Fcc_Compliant(parser_week, eas_handles,month, trace_flag):
	...
	if month.RmtCheckNecessary(parser_week) == True:
		if month.rmt == False:
			eas_handles.WriteToOutFile("{\\large{WARNING!!! No RMT sent during "+str(parser_week.sunday.month)+"/"\
				+str(parser_week.sunday.year)+"}}\n\n")
