...
#---------------------------------------MIGHT NOT BE NECESSARY--------------------------------------------------#
		...
		if ($line =~ m/Matched Filter/)
		{
			$line = <INHANDLE>;
			chomp($line); #Added out of habit...
		}

		if($line =~ m/Already heard/)
		{
...
#---------------------------------------------------------------------------------------------------------------#
			...
			if (!eof(INHANDLE) && $Rec_Snd eq "Received")
			{
				if($line =~ m/^EOM/)
				{
		...
		#---------------------------------------------------------------------------------------------------------------#
			...
			elsif(!eof(INHANDLE))
			{
				...
				{
					printf OUTHANDLE ("\\hspace{20pt}     (Local alert) %s %s from %s at %s/%s/%s %s \\\\",
						   $Rec_Snd,$EAStype,$EASwhere,$month,$day,$year,$time);
				}
				if($EAStype eq "RWT")
				{
		#---------------------------------------CHANGE BROADCAST STATION MESSAGE, IF NEEDED-----------------------------#
