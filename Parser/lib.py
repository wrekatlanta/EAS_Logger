...
class	Month:
	...
	def 	RmtAddToPDFIfNeeded(self, eas_handles):
		if self.RmtCheckNecessary() == True:
			eas_handles.WriteToOutFile("\\medskip\n")
			# RmtCheckNecessary() allows us to ignore checks for leap year months so a vague condition is good enough.
			if self.requested_week.saturday.day >= 21 and self.requested_week.saturday.day <=31:
				eas_handles.WriteToOutFile("\n\\hspace{20 pt} RMT received for "+str(self.requested_week.saturday.month)+"/"+str(self.requested_week.saturday.year)+"\n\n")
			else:
				# Start of a new year
				if self.requested_week.saturday.month == 1:
					eas_handles.WriteToOutFile("\n\\hspace{20 pt} RMT received for "+str(self.requested_week.saturday.month-1)+"/"+str(self.requested_week.saturday.year-1)+"\n\n")
				else:
					eas_handles.WriteToOutFile("\n\\hspace{20 pt} RMT received for "+str(self.requested_week.saturday.month-1)+"/"+str(self.requested_week.saturday.year)+"\n\n")
			eas_handles.WriteToOutFile("\\medskip\n")

	def	RmtCheckForTraceFile(self, week, eas_handles, trace_flag):
		...
		if month_table[week.saturday.month][self.LeapYear(week.saturday.year)] == 28:
			if week.sunday.day >= 21 and (week.saturday.day <= 7 or week.saturday.day == 28 ):
				if trace_flag == True:
					eas_handles.WriteTo_TraceFile("\nRmt check is necessary\n")
				return True
		if month_table[week.saturday.month][self.LeapYear(week.saturday.year)] == 29:
			if week.sunday.day >= 22 and (week.saturday.day <= 7 or week.saturday.day == 29 ):
				if trace_flag == True:
					eas_handles.WriteTo_TraceFile("\nRmt check is necessary\n")
				return True
		if trace_flag == True:
			eas_handles.WriteTo_TraceFile("\nRmt check is not necessary\n")
		return False

	def	RmtCheckNecessary(self):
		if month_table[self.requested_week.saturday.month][self.LeapYear(self.requested_week.saturday.year)] == 30:
			if self.requested_week.sunday.day >= 23 and (self.requested_week.saturday.day <= 7 or self.requested_week.saturday.day == 30 ):
				return True
		if month_table[self.requested_week.saturday.month][self.LeapYear(self.requested_week.saturday.year)] == 31:
			if self.requested_week.sunday.day >= 23 and (self.requested_week.saturday.day <= 7 or self.requested_week.saturday.day == 31 ):
				return True
		# Accounting for February leap years and otherwise
		if month_table[self.requested_week.saturday.month][self.LeapYear(self.requested_week.saturday.year)] == 28:
			if self.requested_week.sunday.day >= 21 and (self.requested_week.saturday.day <= 7 or self.requested_week.saturday.day == 28 ):
				return True
		if month_table[self.requested_week.saturday.month][self.LeapYear(self.requested_week.saturday.year)] == 29:
			if self.requested_week.sunday.day >= 22 and (self.requested_week.saturday.day <= 7 or self.requested_week.saturday.day == 29 ):
				return True
		return False

	def	SearchForRmt(self, alert, eas_handles, week, trace_flag):
		if trace_flag == True:
			eas_handles.WriteTo_TraceFile("\nSearchForRmt()\n")
		if self.RmtCheckNecessary() == True:
			if trace_flag == True:
				eas_handles.WriteTo_TraceFile("\nRmtCheckNecessary() Passed\n")
			alert.GetAlertDetails(eas_handles, trace_flag)
			alert.ExtractInfoFromZCZC(eas_handles, week, trace_flag)
			if "RMT" in alert.eastype:
				if trace_flag == True:
					eas_handles.WriteTo_TraceFile("\nRMT found!\n")
				self.rmt = True

	def 	Rmt_CheckAndSearch(self, week, alert, eas_handles, trace_flag):
		if trace_flag == True:
			eas_handles.WriteTo_TraceFile("\nRmt_CheckAndSearch()\n")
		if self.RmtCheckNecessary() == True:
			if "RMT" in alert.eastype:
	...
	def 	CheckIf_RmtOutsideWeekRange(self, eas_handles, week, requested_week, trace_flag):
		if self.RmtCheckNecessary() == True:
			if trace_flag == True:
				eas_handles.WriteTo_TraceFile("Rmt Check is necessary\n")
			date_regex = "(\d{2})\/(\d{2})\/(\d{2})"
			alert = Alert()
			...
			if alert.curr_date.month == requested_week.sunday.month and alert.curr_date.year == requested_week.sunday.year:
				if trace_flag == True:
					eas_handles.WriteTo_TraceFile("\nRMT check month reached!\n")
				if re.search("(Matched Filter)|(Already heard)", eas_handles.eas_infile_line):
					eas_handles.IncrementIndexByOne()
				self.SearchForRmt(alert,eas_handles,week, trace_flag)
				if trace_flag == True and self.rmt == True:
					eas_handles.WriteTo_TraceFile("\nRMT found!\n")
			elif trace_flag == True:
				eas_handles.WriteTo_TraceFile(str(alert.curr_date.month)+" does not match "+str(requested_week.sunday.month)+"\n" )
...
class	Alert:
	...
	def	GetAlertDetails(self, eas_handles,trace_flag):
		if trace_flag == True:
			eas_handles.WriteTo_TraceFile("\nGetAlertDetails()\n")
	...
	def 	SetReceivedOrSent(self, eas_handles):
		if "Received" in eas_handles.eas_infile_line:
			...
			regex = re.search("monitor #(\d)|(CAP)",eas_handles.eas_infile_line)
			if(regex.group(1) == None):
				self.monitor = regex.group(2)
			else:
				self.monitor = regex.group(1)
	...
	def	ExtractInfoFromZCZC(self, eas_handles, week, trace_flag):
		if trace_flag == True:
			eas_handles.WriteTo_TraceFile("\nExtractInfoFromZCZC\n")
		...
		if re.search("([A-Z ]{7,8}-$)", self.zczc):
			...
			eas_handles.IncrementIndexByOne()
		if trace_flag == True:
			eas_handles.WriteTo_TraceFile("\nself.eastype="+self.eastype+"\nself.easwhere="+self.easwhere+"\nself.zczc="+self.zczc+"\n")
	...
	def	Check_For_Eom_And_Display_Received_Info(self, eas_handles):
		if re.search("^EOM",eas_handles.eas_infile_line):
			...
			if(self.monitor == "CAP"):
				eas_handles.WriteToOutFile("\\hspace{20 pt}     "+self.rec_or_sent+" "+self.eastype+" from CAP at "\
					+FormattedDateString(self.curr_date)+" "+self.curr_time+" (EOM Received at "+self.eom_date+" "\
					+self.eom_time+" from CAP \\\\")
			else:
			...
			if "Received" in self.rec_or_sent:
				if(self.monitor == "CAP"):
					eas_handles.WriteToOutFile("\\hspace{20 pt}     "+self.rec_or_sent+" "+self.eastype+" from CAP at "\
						+FormattedDateString(self.curr_date)+" "+self.curr_time+" from CAP \\\\")
		...
		if re.search("RWT", self.eastype) and not (self.monitor == "CAP"):
			eas_handles.WriteToOutFile("\\hspace{20pt}     A broadcast station or cable system has issued a required weekly test"\
						 +"\n\n\\medskip\n\n")
		elif (re.search("RWT", self.eastype)):
			eas_handles.WriteToOutFile("\\hspace{20pt}     The Civil Authorities have issued a required weekly test"\
						 +"\n\n\\medskip\n\n")
		else:
	...
	def	FlagRelevantAlerts(self,week):
		...
		if "WREK" in self.easwhere:
			week.rwt = True
		if "CAP" in self.monitor:
			week.cap_received = True
...
def	FormattedDateString(date):
	...
	return str(datestring[5:7]) +"/"+ str(datestring[8:10]) +"/"+ str(datestring[2:4])
...
def	Perform_Trace_Extraction( week, alert, eas_handles, month,requested_week):
	...
	if month.RmtCheckNecessary() == True:
		eas_handles.WriteTo_TraceFile("Rmt flag is-> "+str(month.rmt)+"\n")
...
def	Fcc_Compliant(parser_week, eas_handles,month, trace_flag):
	...
	if month.RmtCheckNecessary() == True:
		if month.rmt == False:
			eas_handles.WriteToOutFile("{\\large{WARNING!!! No RMTs sent during "+str(parser_week.sunday.month)+"/"\
				+str(parser_week.sunday.year-2000)+"}}\n\n")
	if(parser_week.cap_received == False):
		eas_handles.WriteToOutFile("{\\large{WARNING!!! No CAP alerts received during "+FormattedDateString(parser_week.sunday)+" - "\
			+FormattedDateString(parser_week.saturday) + " }}\n\n")
