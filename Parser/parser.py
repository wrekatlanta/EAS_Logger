...
day_after_requested_saturday = lib.datetime.date(requested_week.saturday.year,requested_week.saturday.month,requested_week.saturday.day ) +\
					lib.datetime.timedelta(1)

# File names used
infile = "eas.log"
# Format the output file name to be named after the date following the saturday of that week
outfile = "easlog_" + lib.FormattedDateString( day_after_requested_saturday ) + ".tex"
	...
	if lib.re.search("((Alert Received)|(Alert sent)|(Local Alert))", eas_handles.eas_infile_line )\
		...
		if requested_week == week:
			...
			while requested_week == week and not eas_handles.EndOfList():
				...
				alert.ExtractInfoFromZCZC(eas_handles, requested_week, trace_flag)

				# RMT checks
				month.Rmt_CheckAndSearch(week, alert, eas_handles, trace_flag)
...
month.RmtAddToPDFIfNeeded(eas_handles)
